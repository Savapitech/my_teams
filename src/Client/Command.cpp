...
void LoginCommand::logCommand(const std::string &serverResponse) {
    ...
    if (status == 200) {
        ...
        if (args.size() >= 2)
            client_event_logged_in(args[0].c_str(), args[1].c_str());
    } else if (status == 401) {
...
void LogoutCommand::logCommand(const std::string &serverResponse) {
    ...
    if (status == 200) {
        std::vector<std::string> args = extractArgs(serverResponse);
        if (args.size() >= 2)
            client_event_logged_out(args[0].c_str(), args[1].c_str());
    }
}
...
void UsersCommand::logCommand(const std::string &serverResponse) {
    ...
    if (status == 200) {
        ...
        for (size_t i = 0; i + 2 < args.size(); i += 3) {
            client_print_users(args[i].c_str(), args[i+1].c_str(), std::stoi(args[i+2]));
        }
    }
}
...
void SendCommand::logCommand(const std::string &serverResponse) {
    ...
    if (status == 404) {
        std::vector<std::string> args = extractArgs(serverResponse);
        if (!args.empty()) client_error_unknown_user(args[0].c_str());
    }
}
...
void MessagesCommand::logCommand(const std::string &serverResponse) {
    ...
    if (status == 200) {
        for (size_t i = 0; i + 2 < args.size(); i += 3) {
            ...
            client_private_message_print_messages(args[i].c_str(), timestamp, args[i+2].c_str());
        }
...
void SubscribedCommand::logCommand(const std::string &serverResponse) {
    ...
    if (status == 200) {
        if (serverResponse.find("TEAM") != std::string::npos) {
            for (size_t i = 0; i + 2 < args.size(); i += 3) {
                client_print_teams(args[i].c_str(), args[i+1].c_str(), args[i+2].c_str());
            }
        }
...
void UseCommand::logCommand(const std::string &serverResponse) {
    ...
    if (status == 404 && !args.empty()) {
        if (serverResponse.find("TEAM") != std::string::npos) client_error_unknown_team(args[0].c_str());
        else if (serverResponse.find("CHANNEL") != std::string::npos) client_error_unknown_channel(args[0].c_str());
        else if (serverResponse.find("THREAD") != std::string::npos) client_error_unknown_thread(args[0].c_str());
    }
}
...
void CreateCommand::logCommand(const std::string &serverResponse) {
    ...
    if (status == 201) {
        if (serverResponse.find("TEAM") != std::string::npos && args.size() >= 3)
            client_print_team_created(args[0].c_str(), args[1].c_str(), args[2].c_str());
        else if (serverResponse.find("CHANNEL") != std::string::npos && args.size() >= 3)
            client_print_channel_created(args[0].c_str(), args[1].c_str(), args[2].c_str());
        else if (serverResponse.find("THREAD") != std::string::npos && args.size() >= 5)
            client_print_thread_created(args[0].c_str(), args[1].c_str(), std::stoll(args[2]), args[3].c_str(), args[4].c_str());
        else if (serverResponse.find("REPLY") != std::string::npos && args.size() >= 4)
            client_print_reply_created(args[0].c_str(), args[1].c_str(), std::stoll(args[2]), args[3].c_str());
    } 
    else if (status == 409) {
...
void ListCommand::logCommand(const std::string &serverResponse) {
    ...
    if (status == 200) {
        if (serverResponse.find("TEAM") != std::string::npos) {
            ...
            for (size_t i = 0; i + 2 < args.size(); i += 3)
                client_team_print_channels(args[i].c_str(), args[i+1].c_str(), args[i+2].c_str());
        } else if (serverResponse.find("THREAD") != std::string::npos) {
            for (size_t i = 0; i + 4 < args.size(); i += 5)
                client_channel_print_threads(args[i].c_str(), args[i+1].c_str(), std::stoll(args[i+2]), args[i+3].c_str(), args[i+4].c_str());
        } else if (serverResponse.find("REPLY") != std::string::npos) {
            for (size_t i = 0; i + 3 < args.size(); i += 4)
                client_thread_print_replies(args[i].c_str(), args[i+1].c_str(), std::stoll(args[i+2]), args[i+3].c_str());
        }
    }
}
...
void InfoCommand::logCommand(const std::string &serverResponse) {
    ...
    if (status == 200) {
        if (serverResponse.find("USER") != std::string::npos && args.size() >= 3)
            client_print_user(args[0].c_str(), args[1].c_str(), std::stoi(args[2]));
        else if (serverResponse.find("TEAM") != std::string::npos && args.size() >= 3)
            client_print_team(args[0].c_str(), args[1].c_str(), args[2].c_str());
        else if (serverResponse.find("CHANNEL") != std::string::npos && args.size() >= 3)
            client_print_channel(args[0].c_str(), args[1].c_str(), args[2].c_str());
        else if (serverResponse.find("THREAD") != std::string::npos && args.size() >= 5)
            client_print_thread(args[0].c_str(), args[1].c_str(), std::stoll(args[2]), args[3].c_str(), args[4].c_str());
    }
}